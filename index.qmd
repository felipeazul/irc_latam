...
#      TRUE ~ "#00B8D9",
  ...
  geom_col(alpha = .7) +
  geom_text(aes(
    x = fy, y = clients + 35000, label = label_num),
    colour = "black", size = 4, family = "Inter"
  ) +
  scale_y_continuous(
...
Using these data collected, we run statistical simulations to estimate the total number of clients: around 366,300 clients in FY24. But we also give a range—our best guess is 366,300 and we’re 90% confident that the number for FY24 lies between 352,600 and 388,400.
...
-   [Katie Susman](mailto:katharine.susman@rescue.org), Head of Region, Latam
