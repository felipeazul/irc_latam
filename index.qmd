---
title: "The IRC is rapidly expanding its client reach in Latin America [DRAFT]"
subtitle: "Our model indicates that client reach peaked in FY22. Colombia and Venezuela account for the largest numbers, while Ecuador, Peru and El Salvador are notable fast risers. Health services dominate overall support."
# subtitle: "Client reach peaked in FY22. Colombia and Venezuela lead in absolute numbers, but Ecuador, Peru, and El Salvador are fast risers. Health services dominate overall support."
format: html
---
...
The result: **in FY24 the IRC reached around 331,000 people in Latin America**. Statistically, we are 90% confident the true figure lies between 322,000 and 340,000. (This does not yet include people reached through online or mass-media campaigns such as the Signpost project.)
...
fig <- tags$figure(
  ...
  tags$figcaption(
    "Each line shows the estimated clients per country. From FY19 to FY21, North Central America (NCA) was counted as a single country. From FY22 the numbers were separated out. Click legend items to toggle countries.",
    class = "figure-caption"
  )
)
...
results_mean <- results %>%
  ...
  group_by(fy, region, country, type) %>%
  summarise(clients = mean(clients)) %>%
  separate(
    type,
    into = c("sector", "gender"),
    sep = "_",
...
fig <- tags$figure(
  ...
  tags$figcaption(
    "Each line shows the estimated clients across all countries per sector. Click legend items to toggle sectors.",
    class = "figure-caption"
  )
)
...
Yes! The Annual Statistics is a set of more than 400 figures collected yearly across sectors. Using these figures we can break down the client count by sector and gender. Note that not all indicators are collected by gender. Some indicators are just overall service numbers. This is often the case for informational services, or services like vaccinations. In these cases, the statstical model imputes a gender ratio, based on existing gender ratios at different levels (ie, within a region, within a country, and by sector). Figure 3 shows the average (mean) client count across all simulations, divided by sector, with female, male and total values displayed.
...
chart_dotplot <- dotplot_data %>%
  ...
  labs(x = NULL) +
  scale_x_continuous(
    limits = c(310000, 350000),
    breaks = c(310000, 320000, 330000, 340000, 350000),
    labels = scales::label_number(scale_cut = scales::cut_short_scale(), accuracy = 1),
  ...
  theme(
    ...
    axis.text.x = element_text(family = "Roboto Mono", size = 32)
  ) +
  annotate(
    "curve", 
...
ggsave("chart_dotplot.png", chart_dotplot, width = 8, height = 4, dpi = 300, bg = "white")
knitr::include_graphics("chart_dotplot.png")
